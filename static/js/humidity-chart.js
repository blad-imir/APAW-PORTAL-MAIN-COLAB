/**
 * Humidity Chart - Hourly and daily humidity visualization
 * Reads configuration from window.APP_CONFIG (injected by Flask from config.py)
 */

class HumidityChart {
	constructor(config) {
		this.chartId = config.chartId || "humidityChart";
		this.apiEndpoint = config.apiEndpoint || "/api/humidity-data";
		...
		if (
			...
			throw new Error("[HUMIDITY] stationConfig is required!");
		}
	...
	_cacheElements() {
		this.elements = {
			...
			loading: document.getElementById("humidity-chart-loading"),
			error: document.getElementById("humidity-chart-error"),
			datePicker: document.getElementById("humidityDatePicker"),
		};
	}
	...
	loadSpecificDate(dateString) {
		...
		this._loadDebounceTimer = setTimeout(() => {
			this._loadHumidityData(dateString);
		}, 300);
	...
	async _loadDateRange() {
		try {
			const response = await fetch("/api/humidity-date-range");
			...
			if (!response.ok) {
				...
				try {
					...
					console.warn("[HUMIDITY] Could not parse error response as JSON");
				}
	...
	async _loadHumidityData(dateString = null) {
		...
		try {
			...
			if (!response.ok) {
				...
				try {
					...
					console.error("[HUMIDITY] Non-JSON error response:", parseError);
			...
			if (attempt < MAX_RETRIES) {
				...
				this._showError("Failed to load humidity data. Please try again later.");
			}
		}
	}
	...
	_formatTooltip(e) {
		...
		let html = `<div class="humid-tooltip" style="width: ${tooltipWidth};">`;
		...
		e.entries.forEach((entry) => {
			...
			if (count === 0) {
				...
				valueColor = "#94a3b8";
			} else if (humidity === null || isNaN(humidity)) {
				displayValue = "--";
				valueColor = "#94a3b8";
			} else {
				displayValue = `${humidity.toFixed(1)} %`;
				valueColor = color;
			}

			html += `
				<div class="humid-tooltip__row">
					<span class="humid-tooltip__label">${name}</span>
					<span class="humid-tooltip__value" style="color: ${valueColor}; font-weight: 600;">
						${displayValue}
					</span>
				</div>
			`;
